...
**核心使命**：
在架构更新 (`.vibe/genesis/v{N}`) 之前或之后，探测系统风险、暗坑和耦合。
Scout 的发现将作为**输入**反馈给 Architectural Overview。

**Output Goal**: `.vibe/genesis/v{N}/00_SCOUT_REPORT.md`
</phase_context>
...
> 
> 判断方式: 如果 `.vibe/genesis/v{N}/concept_model.json` 存在 → 模式 B，Step 5 执行对比  
...
2.  **获取当前架构上下文** (Optional):
    - 如果存在 `.vibe/genesis/v{N}`，读取其 `02_ARCHITECTURE_OVERVIEW.md` 以对比"计划"与"现实"。
...
2.  **对比**: 将代码中的概念与 `.vibe/genesis/v{N}/concept_model.json` 进行对比 (如果存在)。
...
1.  **如果你正在服务于一次 Genesis 更新**:
    - 结合 `.vibe/genesis/v{N}/01_PRD.md` (新需求)。
...
保存报告到 `.vibe/genesis/v{N}/00_SCOUT_REPORT.md`

（注意：如果版本不存在，默认为 v1）

确保 `.vibe/genesis/v{N}/` 目录存在。
