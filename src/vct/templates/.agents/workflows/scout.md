...
**核心使命**：
在架构更新 (`.antigravity/genesis/v{N}`) 之前或之后，探测系统风险、暗坑和耦合。
Scout 的发现将作为**输入**反馈给 Architectural Overview。

**Output Goal**: `.antigravity/genesis/v{N}/00_SCOUT_REPORT.md`
</phase_context>
...
> 
> 判断方式: 如果 `.antigravity/genesis/v{N}/concept_model.json` 存在 → 模式 B，Step 5 执行对比  
...
Scout 的侦察对象默认是 `TARGET_PROJECT`，而不是控制面板仓库本身。
...
4. 向用户回显：
   - 控制面板目录
   - 侦察目标目录（`TARGET_PROJECT`）
5. 后续所有“扫描根目录”“构建系统”“运行时通信”“Git 历史”分析都针对 `TARGET_PROJECT`

如果用户明确要侦察控制面板仓库本身，应先说明并切换范围；否则默认侦察 `TARGET_PROJECT`。

---

## Step 1: 建立大局观 (System Fingerprint)

1.  **扫描目标代码仓库根目录**: 列出 `TARGET_PROJECT` 根目录下的所有文件和文件夹
2.  **获取当前架构上下文** (Optional):
    - 如果存在 `.antigravity/genesis/v{N}`，读取其 `02_ARCHITECTURE_OVERVIEW.md` 以对比"计划"与"现实"。
...
2.  **对比**: 将代码中的概念与 `.antigravity/genesis/v{N}/concept_model.json` 进行对比 (如果存在)。
...
1.  **如果你正在服务于一次 Genesis 更新**:
    - 结合 `.antigravity/genesis/v{N}/01_PRD.md` (新需求)。
...
保存报告到 `.antigravity/genesis/v{N}/00_SCOUT_REPORT.md`

（注意：如果版本不存在，默认为 v1）

确保 `.antigravity/genesis/v{N}/` 目录存在。
