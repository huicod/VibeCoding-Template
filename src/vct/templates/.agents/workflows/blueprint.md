...
**核心使命**：
读取最新的架构版本 (`.vibe/genesis/v{N}`)，将其拆解为**可执行的任务清单**。
...
**Output Goal**: `.vibe/genesis/v{N}/05_TASKS.md`
</phase_context>
...
1.  **保存**: 将内容保存到 `.vibe/genesis/v{N}/05_TASKS.md`
2.  **验证**: 确保文件包含所有任务、验收标准和依赖图。
3.  **更新 AGENTS.md "当前状态"**:
    - 活动任务清单: `.vibe/genesis/v{N}/05_TASKS.md`
    - 最近一次更新: `{Today}`
...
```markdown
### 当前任务状态
- 任务清单: .vibe/genesis/v{N}/05_TASKS.md
