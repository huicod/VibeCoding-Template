...
**核心使命**：
读取最新的架构版本 (`.antigravity/genesis/v{N}`)，将其拆解为**可执行的任务清单**。
...
**Output Goal**: `.antigravity/genesis/v{N}/05_TASKS.md`
</phase_context>
...
2.  **确定最新版本**:
    ...
    - **TARGET_DIR** = `.antigravity/genesis/v{N}`。
...
1.  **保存**: 将内容保存到 `.antigravity/genesis/v{N}/05_TASKS.md`
2.  **验证**: 确保文件包含所有任务、验收标准和依赖图。
3.  **更新 AGENTS.md "当前状态"**:
    - 活动任务清单: `.antigravity/genesis/v{N}/05_TASKS.md`
    - 最近一次更新: `{Today}`
...
```markdown
### 当前任务状态
- 任务清单: .antigravity/genesis/v{N}/05_TASKS.md
