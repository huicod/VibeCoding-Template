...
部署前先快速扫描 `.vibe/artifacts/error_journal.md` 中与发布、迁移、环境配置相关的 Prevention Rules。
...
无论成功或失败，都将部署过程摘要保存到 `.vibe/artifacts/logs/deploy_[env]_[timestamp].md`。

> **失败处理**：如果任何步骤失败，停止部署、报告错误、记录到 `.vibe/artifacts/error_journal.md`、并在部署日志中附上失败步骤与修复建议。
