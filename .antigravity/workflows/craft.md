...
**支持的输出位置**:
- Workflow → `.antigravity/workflows/[name].md`
- Skill → `.antigravity/skills/[name]/SKILL.md`
...
> [!IMPORTANT]
> 你**必须**进行 **3-5 步**结构化思考整合调研结果；如宿主支持，可使用顺序思考工具。
...
> **输出路径**: [明确说明产出文件的保存路径，包含版本化路径变量 `.antigravity/genesis/v{N}/` 如适用]
...
> [!IMPORTANT]
> 你**必须**进行 **3-5 步**结构化思考；如宿主支持，可使用顺序思考工具。
> 复杂模式可以循环多次。
>
> **为什么？** 内容质量决定工具质量。
...
> [!IMPORTANT]
> 你**必须**进行 **3-5 步**结构化批判性思考；如宿主支持，可使用顺序思考工具。
> 发现问题时可以再次循环深入分析。
>
...
6. "有没有隐含假设没有说明？"
7. "与调研中的最佳实践对比，还缺什么？"
8. "这个工作流/技能，能达到 .antigravity/genesis/challenge 的质量标准吗？"
...
**输出前检查**:
- [ ] 所有路径引用正确（特别是版本化路径 `.antigravity/genesis/v{N}/`）
- [ ] 输出位置在文档中明确声明
