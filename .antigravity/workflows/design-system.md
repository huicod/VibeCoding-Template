...
**Output Goal**: `.antigravity/genesis/v{N}/04_SYSTEM_DESIGN/{system-id}.md`
</phase_context>
...
**检查**:
- [ ] `.antigravity/genesis/v{N}/01_PRD.md` 存在
- [ ] `.antigravity/genesis/v{N}/02_ARCHITECTURE_OVERVIEW.md` 存在
- [ ] `.antigravity/genesis/v{N}/03_ADR/` 存在
...
### 1.2 加载PRD
读取 `.antigravity/genesis/v{N}/01_PRD.md`
...
读取 `.antigravity/genesis/v{N}/02_ARCHITECTURE_OVERVIEW.md`
...
### 1.4 查找该系统的详细定义
在 `.antigravity/genesis/v{N}/02_ARCHITECTURE_OVERVIEW.md` 中搜索 system-id 相关内容
...
### 1.5 加载相关ADR
扫描 `.antigravity/genesis/v{N}/03_ADR/` 目录

**选择性加载**与该系统相关的ADR，例如：
- 技术栈选择 (`ADR_001_TECH_STACK.md`)
- 认证方式 (`ADR_002_AUTHENTICATION.md`，如果该系统涉及认证)
- 数据库选择 (如果该系统是后端或数据库系统)

读取 `.antigravity/genesis/v{N}/03_ADR/ADR_001_TECH_STACK.md`
...
> [!IMPORTANT]
> 你**必须**进行 3-5 步结构化思考生成上下文摘要；如宿主支持，可使用顺序思考工具。
...
**ADR决策**:
- ADR_001: React + Vite 技术栈
- ADR_002: JWT认证（需要在前端存储和发送Token）
```
...
> [!IMPORTANT]
> 你**必须**进行 3-5 步结构化思考完成系统理解；如宿主支持，可使用顺序思考工具。
>
> **为什么？** 深度理解是良好设计的前提。
...
**产出**:
- 调研报告自动保存到: `.antigravity/genesis/v{N}/04_SYSTEM_DESIGN/_research/{system-id}-research.md`
...
## Step 4: 设计 (Structured Design Reasoning)

**目标**: 基于调研和上下文，深度设计系统架构

> [!IMPORTANT]
> 你**必须**进行 5-7 步结构化设计思考；如宿主支持，可使用顺序思考工具。
>
...
**加载 L0 模板**（必须）:
读取 `.antigravity/skills/system-designer/references/system-design-template.md`

**加载 L1 模板**（如果 5.0 决策为「需要」）:
读取 `.antigravity/skills/system-designer/references/system-design-detail-template.md`
...
**保存 L0 文件**（必须）:
将内容保存到 `.antigravity/genesis/v{N}/04_SYSTEM_DESIGN/{system-id}.md`

**保存 L1 文件**（如果 5.0 决策为「需要」）:
将内容保存到 `.antigravity/genesis/v{N}/04_SYSTEM_DESIGN/{system-id}.detail.md`

**示例路径**:
- L0: `.antigravity/genesis/v{N}/04_SYSTEM_DESIGN/core.md`
- L1: `.antigravity/genesis/v{N}/04_SYSTEM_DESIGN/core.detail.md`
...
**调用方式**:
```
/challenge .antigravity/genesis/v{N}/04_SYSTEM_DESIGN/{system-id}.md
```
...
```
✅ 系统设计文档已生成:
  - 文件: .antigravity/genesis/v{N}/04_SYSTEM_DESIGN/{system-id}.md
  - 调研: .antigravity/genesis/v{N}/04_SYSTEM_DESIGN/_research/{system-id}-research.md
...
```markdown
### 系统边界
- {system-id}: {一句话职责} — 详细设计见 `.antigravity/genesis/v{N}/04_SYSTEM_DESIGN/{system-id}.md`
```
