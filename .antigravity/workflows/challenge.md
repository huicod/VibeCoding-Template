...
**Output Goal**: `.antigravity/genesis/v{N}/07_CHALLENGE_REPORT.md`
...
1.  **扫描版本**: `list_dir .antigravity/genesis/`
2.  **确定最新版本**: 找到数字最大的文件夹 `v{N}`（例如 `v3`）。
3.  **TARGET_DIR** = `.antigravity/genesis/v{N}`。
...
> [!IMPORTANT]
> 你**必须**进行 **3-5 步**结构化深度思考；如宿主支持，可使用顺序思考工具。
> 
